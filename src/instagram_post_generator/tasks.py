	...
	def product_analysis(self, agent, product_website, product_details):
		return Task(
			description=dedent(f"""\
				...
				"""),
			expected_output="A detailed product analysis report including key selling points, market appeal, and positioning suggestions.",
			agent=agent
		)
	...
	def competitor_analysis(self, agent, product_website, product_details):
		return Task(
			description=dedent(f"""\
				...
				"""),
			expected_output="A comprehensive competitor analysis report comparing the product with top 3 competitors.",
			agent=agent
		)
	...
	def campaign_development(self, agent, product_website, product_details):
		return Task(
			description=dedent(f"""\
				...
				"""),
			expected_output="A detailed marketing campaign strategy with creative content ideas.",
			agent=agent
		)
	...
	def instagram_ad_copy(self, agent):
		return Task(
			description=dedent("""\
				...
				"""),
			expected_output="Three engaging Instagram ad copy options optimized for the target audience.",
			agent=agent
		)
	...
	def take_photograph_task(self, agent, copy, product_website, product_details):
		return Task(
			description=dedent(f"""\
				...
				"""),
			expected_output="Three detailed photograph descriptions following the specified format.",
			agent=agent
		)
	...
	def review_photo(self, agent, product_website, product_details):
		return Task(
			description=dedent(f"""\
				...
				"""),
			expected_output="Three reviewed and approved photograph descriptions with detailed feedback.",
			agent=agent
		)